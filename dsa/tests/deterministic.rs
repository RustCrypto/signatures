...
fn decode_hex_number(txt: &str, precision: u32) -> BoxedUint {
    let val = hex::decode(txt).unwrap();
    BoxedUint::from_be_slice(&val, precision).unwrap()
}
...
fn dsa_1024_signing_key() -> SigningKey {
    ...
    let p = decode_hex_number(p_str, 1024);

    let q_str = "996F967F6C8E388D9E28D01E205FBA957A5698B1";
    let q = decode_hex_number(q_str, 1024);
    ...
    let g = decode_hex_number(g_str, 1024);

    let x_str = "411602CB19A6CCC34494D79D98EF1E7ED5AF25F7";
    let x = decode_hex_number(x_str, 1024);
...
fn dsa_2048_signing_key() -> SigningKey {
    ...
    let q = decode_hex_number("F2C3119374CE76C9356990B465374A17F23F9ED35089BD969F61C6DDE9998C1F", 2048);
    ...
    let x = decode_hex_number("69C7548C21D0DFEA6B9A51C9EAD4E27C33D3B3F180316E5BCAB92C933F0E4DBC", 2048);
